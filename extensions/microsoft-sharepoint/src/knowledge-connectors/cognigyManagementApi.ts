 ...
 *
 * Provides listing, deletion, and description-patching of knowledge sources —
 * operations not exposed by the @cognigy/extension-tools SDK — for incremental sync.
 *
...
/**
 * Update the description field of an existing knowledge source.
 * Used to store sync metadata (file hash, SharePoint item ID) after upload.
 *
 * PATCH {apiUrl}/v2.0/knowledgestores/{storeId}/knowledgesources/{sourceId}
 */
export async function patchKnowledgeSourceDescription(
    apiUrl: string,
    apiKey: string,
    storeId: string,
    sourceId: string,
